...
function getDayStatus(summary) {
  ...
  if (summary.remaining <= 0) {
    return { text: '매진', className: 'closed' };
  }

  const ratio = summary.remaining / summary.capacity;
  if (ratio <= 0.25) {
    return { text: '마감임박', className: 'warn' };
  }
...
function renderCalendar() {
  ...
  for (let day = 1; day <= lastDate.getDate(); day += 1) {
    ...
    const summary = calendarSummary[iso] || { reserved: 0, capacity: maxSeats * trips.length, remaining: maxSeats * trips.length };
...
async function loadCalendarSummary() {
  const result = await fetchJSON(`/api/calendar?month=${encodeURIComponent(monthKey(currentMonth))}`);
